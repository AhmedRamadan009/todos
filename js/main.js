    ...
    function addUser() {
        ...
        if(validateEmail() == true && validateUsername() == true && validatePassword() == true)
        {
            ...
            if(!localStorage.getItem("UsersList")) {
                ...
                modalForm.innerHTML =`<div class="badge">Done</div>
...
// render all todos in index.html
  function displayTodos() {
    var newTodo = "";
  
    for (var i = 0; i < todos.length; i++) {
      newTodo += ` <div  class="col-md-4 my-4">
      <div class="note p-4">
          <h3 class=""> Title : ${todos[i].title}</h3>
        
        
          <p class="fw-bolder"> Status: ${todos[i].completed }</p>
          <a  data-bs-toggle="modal" data-bs-target="#staticBackdrop2" ><i class="fas fa-edit  edit"></i></a>
          <a onclick="deleteTodo(${i})" > <i class="fas fa-trash-alt px-3 del"></i></a>
      </div>
  </div>`;
    }
    document.getElementById("row-body").innerHTML = newTodo;
  };
...
document.getElementById("added-succes").innerHTML=`<div class="alert alert-success text-center" role="alert"  >
Your Todo Added Sucseefully with Title:${todoTitle.value}
...
updateTodo.addEventListener("click",function (){
  ...
  .then((response) => response.json())
  .then((json) => console.log(json));
document.getElementById("added-succes").innerHTML=`<div class="alert alert-primary text-center" role="alert"  >
Your Todo Updated Sucseefully with Title:${todoTitle.value}
